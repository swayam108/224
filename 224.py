...
def semantic_lidar_data():
    ...
    for detection in point_cloud_data :
        ...
        for object_name in object_id:
            if distance_name_data["name"] == object_name and distance_name_data["distance"] >5 and distance_name_data["distance"] <6:
                dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
                dropped_vehicle.apply_control(carla.VehicleControl(throttle=0.31))
                dropped_vehicle.apply_control(carla.VehicleLightState(carla.VehicleLightState.Brake | carla.VehicleLightState.LowBeam))

                print(f"objest near our car is {object_name} in the distance of {name_distance_data['distance']}")
                break
            else:
                print(f"objest near our car is {object_name} in the distance of {name_distance_data['distance']}")
